...
class GaussianDiffusion:
    ...
    def p_sample_loop_anomaly(self, data_t, denoise_fn, device, noise_fn=torch.randn,
                             constrain_fn=lambda x, t: x, clip_denoised=True, max_timestep=None, keep_running=False):
        """
        Generate samples
        keep_running: True if we run 2 x num_timesteps, False if we just run num_timesteps
        ...
        for t in reversed(range(0, final_time if not keep_running else len(self.betas))):
            data_t = constrain_fn(data_t, t)
            t_ = torch.empty(shape[0], dtype=torch.int64, device=device).fill_(t)
            data_t = self.p_sample(denoise_fn=denoise_fn, data=data_t, t=t_, noise_fn=noise_fn,
                                    clip_denoised=clip_denoised, return_pred_xstart=False).detach()

        assert data_t.shape == shape
        return data_t
...
class Model(nn.Module):
    ...
    def gen_samples_anomaly(self, data_t, device, noise_fn=torch.randn, constrain_fn=lambda x, t: x,
                    clip_denoised=False, max_timestep=None,
                    keep_running=False):
            return self.diffusion.p_sample_loop_anomaly(data_t, self._denoise, device=device, noise_fn=noise_fn,
                                                    constrain_fn=constrain_fn,
                                                    clip_denoised=clip_denoised, max_timestep=max_timestep,
                                                    keep_running=keep_running)
...
def generate(model, opt, outf_syn):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            data = data['test_points'].transpose(1, 2)
            data = data.to('cuda')
            ...
            pcd.points = o3d.utility.Vector3dVector(data[0].T.detach().cpu().numpy())
            o3d.io.write_point_cloud("generate_x_0_trial.ply", pcd)

            # visualize_pointcloud_batch(os.path.join(str(Path(opt.eval_path).parent), 'x.png'), gen[:64], None,
            #                            None, None)

            visualize_pointcloud_batch('%s/generate_i_%03d_x_0.png' % (outf_syn, i), data.transpose(1, 2), None, None, None)
            visualize_pointcloud_batch('%s/generate_i_%03d_x_hat_0.png' % (outf_syn, i), gen.transpose(1, 2), None, None, None)
...
def main_pipeline(anomolous_data, model, opt, i, outf_syn):
    # Get the anomalous point-cloud (batch)
    x_0 = anomolous_data['test_points'].transpose(1, 2)
    x_0 = x_0.to('cuda')

    # Get the mean and std of the anomalous point-cloud (batch)
    m, s = x_0.mean(dim=2, keepdims=True).float().to('cuda'), x_0.std(dim=2, keepdims=True).float().to('cuda')
    ...
    # Visualize x_0 and x̂_0 and save them
    pcd = o3d.geometry.PointCloud()
    pcd.points = o3d.utility.Vector3dVector(x_0[0].T.detach().cpu().numpy())
